...
START_CRAM_TO_BAM_FULL=`date '+%s'` # capture time process starts for wall clock tracking purposes.
...
END_CRAM_TO_BAM_FULL=`date '+%s'` # capture time process starts for wall clock tracking purposes.

# write out timing metrics to file

	echo $SM_TAG"_"$PROJECT",F.01,CRAM_TO_BAM_FULL,"$HOSTNAME","$START_CRAM_TO_BAM_FULL","$END_CRAM_TO_BAM_FULL \
